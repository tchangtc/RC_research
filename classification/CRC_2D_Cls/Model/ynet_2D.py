...
class ContBatchNorm2d(nn.modules.batchnorm._BatchNorm):
    def _check_input_dim(self, input):
        ...
        if input.dim() != 4:
            raise ValueError('Expected 54 input (got {}D input)'.format(input.dim()))
...
class ClassificationHead(nn.Module):
    def __init__(self, in_chan=1, act='prelu', lst_act='sigmoid', cls_classes=1):
        ...
        self.conv1 = nn.Conv2d(in_chan, 1024, kernel_size=3, padding=1)
        self.bn1 = ContBatchNorm2d(1024)
...
class Classification2D(nn.Module):
    def __init__(self, act='relu', lst_act='soft', cls_classes=1):
        super(Classification2D, self).__init__()
